...
import marketing_random_data_ja from "../datas/marketing_random_data_ja.json"
...
const SideItems = ({ onItemClick }: SideItemsProps) => {
  const dataFiles = [
    // { name: "マーケティングデータ", data: data1 },
    { name: "マーケティング日本語ランダムデータ", data: marketing_random_data_ja },
  ...
  return (
    <Box overflow={"hidden"}>
      ...
      >
        <ListItem
          button
          onClick={() => onItemClick("https://jsonplaceholder.typicode.com/posts")}
          sx={{
            display: "flex",
        ...
        {dataFiles.map((dataFile) => (
          <React.Fragment key={dataFile.name}>
            ...
            >
              ...
              />
              <ListItemText
                primary={`records: ${dataFile.data.length} datas`}
                sx={{
                  ml: "auto",
                  color: "#666",
                  "&.MuiListItemText-root>.MuiTypography-root": {
